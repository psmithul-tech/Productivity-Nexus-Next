...
export async function GET() {
  ...
  try {
    const [taskRes, eventRes, reminderRes, settingsRes, researchRes] = await Promise.all([
    ...
    const processingResearch = researchRows.find(r => r.status === 'processing')?.count ?? 0;
    const completedResearch = researchRows.find(r => r.status === 'completed')?.count ?? 0;
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        processingResearch: Number(processingResearch),
        completedResearch: Number(completedResearch),
      },
      integrations: {
